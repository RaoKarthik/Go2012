...
/* 		customEvents: [[]], */
		update: function () {
			...
			for (var i=0; i<3; i++) {
				...
				$(this.masterPages[i]).html("<span>"+this.data[page]+"</span>");
				this.masterPages[i].style.visibility = "visible";
			}
			this.directionX = 0;
			this.updatePageCount(this.data.length);
			this.refreshSize();
			//this.goToPage(0);
		},
		onFlip: function (fn) {

			// THIS CAUSES AN UNKNOWN BUG // NOT ANYMORE
			for (var i=0; i<this.customEvents.length; i++) {
				if(this.customEvents[i][0]=="flip"){
					this.wrapper.removeEventListener('swipeview-'+this.customEvents[i][0], this.customEvents[i][1], true);
					//this.wrapper.detachEvent('swipeview-'+this.customEvents[i][0]);
...
/*			if (!hasTouch) {
		...
		goToPage: function (p) {
			...
			if (this.currentMasterPage == 0) {
				...
				this.masterPages[0].style.visibility = "visible";
				...
				this.masterPages[2].style.left = this.page * 100 + 100 + '%';
				this.masterPages[1].style.visibility = "visible";
				...
				this.masterPages[0].style.left = this.page * 100 + 100 + '%';
				this.masterPages[2].style.visibility = "visible";
...
/*			if (newX > 0 || newX < this.maxX) {
			...
			// NICS FIX
			if(newX>0 || newX < -this.pageWidth*(this.data.length-1)){
				return false;
			}
		...
		__checkPosition: function () {
			...
			this.durationLeft = this.slider.style.webkitTransitionDuration = Math.floor(300 * Math.abs(this.x - newX) / this.pageWidth) + 'ms';
